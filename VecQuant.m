...
%VERY FAST WITH THE SMALLER IMAGE AND FINE WITH THE LARGER.

function [J, M] = VecQuant(A, k, maxIter, tol)

    dim = length(A);

    rA = [reshape(A(:,:,1),[1,dim^2]); reshape(A(:,:,2),[1,dim^2]); reshape(A(:,:,3),[1,dim^2])];
    ...
    R = randi(dim,k,2);
    for i = 1:16
        M(i,:) = squeeze(A(R(i,1),R(i,2),:))';
    end
    %------------------------------------------------%
            
    mInd = ones(dim);   %mInd is a 128x128 (512x512, or dimXdim in general) matrix where 
                        %mInd(k,l) is the index of the centroid with the MINIMUM DISTANCE 
                        %to pixel k,l so far seen in the loop. It is initalized to 1, the result 
